(function(){
  ...
  const W={0:['☀️','Ciel dégagé'],1:['🌤️','Plutôt dégagé'],2:['⛅','Partiellement nuageux'],3:['☁️','Couvert'],45:['🌫️','Brouillard'],48:['🌫️','Brouillard givrant'],51:['🌦️','Bruine faible'],53:['🌦️','Bruine'],55:['🌧️','Bruine forte'],61:['🌦️','Pluie faible'],63:['🌧️','Pluie'],65:['🌧️','Pluie forte'],71:['🌨️','Neige faible'],80:['🌦️','Averses'],81:['🌧️','Averses'],82:['⛈️','Fortes averses'],95:['⛈️','Orage'],96:['⛈️','Orage'],99:['⛈️','Orage']};
  const css=document.createElement('style');
  css.textContent='.hero:after{content:"V1.4"!important}.v14wx{margin:14px 0 0;padding:12px 14px;border-radius:15px;background:#eef7f4;border:1px solid #d4e8e1}.v14wxhead{display:flex;align-items:center;justify-content:space-between;gap:8px}.v14wx strong{font-size:.92rem}.v14wxsmall{font-size:.78rem;color:#60736b}.v14wxgrid{display:grid;grid-template-columns:repeat(4,1fr);gap:7px;margin-top:9px}.v14wxgrid span{background:#fff;border:1px solid #dce6e1;border-radius:10px;padding:8px 5px;text-align:center;font-size:.74rem}.v14swap{margin-top:10px;border-radius:12px;padding:10px 12px;font-size:.78rem;font-weight:750}.v14ok{background:#eaf7ef;color:#22613b}.v14warn{background:#fff1df;color:#7a531b}.v14bad{background:#ffe7e3;color:#8a3028}.v14pending{background:#f4f5f4;color:#66726e}.v14global{max-width:920px;margin:12px auto 0;padding:0 12px}.v14globalbox{background:#fff;border:1px solid #dce6e1;border-radius:18px;padding:14px;box-shadow:0 8px 24px rgba(20,50,40,.07)}.v14globalbox b{color:#0f6f5f}@media(max-width:520px){.v14wxgrid{grid-template-columns:repeat(2,1fr)}}';
  document.head.appendChild(css);
  const footer=document.querySelector('footer'); if(footer) footer.textContent=footer.textContent.replace('V1.3','V1.4');
  const dash=document.querySelector('.v12dash');
  if(dash&&!document.querySelector('.v14global')){const g=document.createElement('section');g.className='v14global';g.innerHTML='<div class="v14globalbox"><b>🌦️ V1.4 · météo intelligente</b><div class="v14wxsmall" id="v14globaltext">Chargement des prévisions locales…</div></div>';dash.insertAdjacentElement('afterend',g);}
  ...
  async function load(d){
    ...
    const box=document.createElement('div'); box.className='v14wx'; box.innerHTML='<div class="v14wxhead"><strong>🌦️ '+d.name+'</strong><span class="v14wxsmall">'+d.date.split('-').reverse().slice(0,2).join('/')+'</span></div><div class="v14wxsmall">Chargement de la prévision…</div>';
    head.appendChild(box);
    try{
      const u='https://api.open-meteo.com/v1/forecast?latitude='+d.lat+'&longitude='+d.lon+'&daily=weather_code,temperature_2m_max,temperature_2m_min,precipitation_probability_max,wind_speed_10m_max,cloud_cover_mean&timezone=Atlantic%2FMadeira&forecast_days=16';
      const r=await fetch(u); if(!r.ok)throw new Error('HTTP '+r.status); const j=await r.json(); const k=j.daily.time.indexOf(d.date);
      if(k<0){box.innerHTML='<div class="v14wxhead"><strong>🌦️ '+d.name+'</strong><span class="v14wxsmall">'+d.date.split('-').reverse().slice(0,2).join('/')+'</span></div><div class="v14swap v14pending">Prévision pas encore disponible pour cette date.</div>';return null;}
      const x={code:j.daily.weather_code[k],max:Math.round(j.daily.temperature_2m_max[k]),min:Math.round(j.daily.temperature_2m_min[k]),precip:Math.round(j.daily.precipitation_probability_max[k]||0),wind:Math.round(j.daily.wind_speed_10m_max[k]||0),cloud:Math.round(j.daily.cloud_cover_mean[k]||0)};
      const wl=W[x.code]||['🌤️','Variable']; const s=score(x,d),L=label(s);
      let swap=''; if(d.sensitive&&s<60&&d.swap?.length)swap='<div class="v14swap '+L[0]+'">'+L[1]+' · alternatives faciles : '+d.swap.map(n=>'J'+n).join(', ')+'</div>'; else swap='<div class="v14swap '+L[0]+'">'+L[1]+'</div>';
      ...
      return {d,x,s};
    }catch(e){box.innerHTML='<div class="v14wxhead"><strong>🌦️ '+d.name+'</strong></div><div class="v14swap v14pending">Météo indisponible pour le moment · le programme reste accessible normalement.</div>';return null;}
  }
  Promise.all(days.map(load)).then(res=>{const a=res.filter(Boolean);const g=document.getElementById('v14globaltext');if(!g)return;if(!a.length){g.textContent='Prévisions indisponibles pour le moment.';return;}const risky=a.filter(o=>o.d.sensitive&&o.s<60);if(!risky.length)g.textContent='Aucune journée météo-sensible ne nécessite de permutation pour l’instant.';else g.textContent='À surveiller : '+risky.map(o=>'J'+o.d.id+' '+o.d.name).join(' · ')+'. Les alternatives sont indiquées dans chaque journée.';});
})();